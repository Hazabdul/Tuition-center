...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: existing } = await supabase.from('teachers').select('id, is_active').eq('id', params.id).eq('institute_id', user.instituteId).is('deleted_at', null).maybeSingle();
    ...
    await logActivity({
      ...
      entityType: 'teacher', entityId: params.id,
      oldValues: existing, newValues: { is_active: isActive }, request,
    });

    return apiSuccess(teacher, `Teacher ${isActive ? 'activated' : 'deactivated'} successfully`);
  } catch (error) {
