...
export async function PATCH(
  ...
  try {
    ...
    if (existing.status === 'suspended') return apiError('Subscription is already suspended', 400);

    await InstituteSubscriptionDoc.findByIdAndUpdate(params.id, {
      $set: { status: 'suspended', performedBy: new mongoose.Types.ObjectId(user.id) },
    });
    ...
    return apiSuccess(null, 'Subscription suspended');
  } catch (error) {
