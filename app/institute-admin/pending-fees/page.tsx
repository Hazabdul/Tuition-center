...
const columns: Column<FeeRow>[] = [
  ...
  { key: 'balance_amount', header: 'Balance', render: (r) => <span className="font-semibold text-amber-700">{formatCurrency(r.balance_amount)}</span> },
  { key: 'due_date', header: 'Due Date', render: (r) => <span>{r.due_date ? formatDate(r.due_date) : '—'}</span> },
  {
    key: 'status',
    header: 'Status',
    render: (r) => (
...
export default function InstituteAdminPendingFeesPage() {
  const api = useApi();
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ['pending-fees', search, page],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), limit: '20', search, status: 'pending' });
  ...
  return (
    <DashboardLayout navSections={instituteAdminNav} role="institute_admin">
      <PageHeader title="Pending Fees" description="Students with outstanding fee balances">
        <div className="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg">
          <AlertCircle className="h-4 w-4" />
          {data?.pagination?.total || 0} pending records
        </div>
      </PageHeader>
