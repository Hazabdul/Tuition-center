      ...
      <DataTable<TeacherRow>
        ...
        toolbar={
          <>
            ...
            <Button onClick={() => setShowCreate(true)}><Plus className="h-4 w-4 mr-2" /> Add Teacher</Button>
          </>
        }
        rowActions={(row) => (
          <DropdownMenu>
...
function CreateTeacherDialog({ open, onOpenChange, onSubmit, isSubmitting }: {
  ...
  return (
    <FormDialog open={open} onOpenChange={onOpenChange} title="Add Teacher" description="Create a new teacher record" onSubmit={() => onSubmit(form)} submitLabel="Create" isSubmitting={isSubmitting} size="lg">
      <div className="grid grid-cols-2 gap-4 py-2">
        ...
        <div className="space-y-1.5"><Label>Specialization</Label><Input value={form.specialization} onChange={(e) => setForm({ ...form, specialization: e.target.value })} placeholder="Mathematics" /></div>
