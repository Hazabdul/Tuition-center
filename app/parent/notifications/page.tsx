...
export default function ParentNotificationsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => { const res = await api.get<NotificationItem[]>('/api/v1/notifications?limit=50'); return res.data || []; },
  });
              ...
              <CardContent className="p-4 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {!n.is_read && <span className="inline-flex h-2 w-2 rounded-full bg-blue-500 flex-shrink-0 mt-1.5" />}
                  </div>
                  <p className="text-sm text-slate-600 mt-0.5">{n.message}</p>
                  <p className="text-xs text-slate-400 mt-1">{format(new Date(n.created_at), 'MMM d, yyyy h:mm a')}</p>
                </div>
              </CardContent>
            </Card>
          ))}
