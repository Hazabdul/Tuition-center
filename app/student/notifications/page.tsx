...
export default function StudentNotificationsPage() {
  ...
  return (
    <DashboardLayout navSections={studentNav} role="student">
      ...
      {isLoading ? (
        <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-20 bg-slate-100 rounded-lg animate-pulse" />)}</div>
      ) : !data || data.length === 0 ? (
              ...
              <CardContent className="p-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-sm font-medium ${!notification.is_read ? 'text-slate-900' : 'text-slate-700'}`}>{notification.title}</p>
                    {!notification.is_read && <span className="inline-flex h-2 w-2 rounded-full bg-blue-500 flex-shrink-0 mt-1.5" />}
                  </div>
                  <p className="text-sm text-slate-600 mt-0.5">{notification.message}</p>
                  <p className="text-xs text-slate-400 mt-1">{format(new Date(notification.created_at), 'MMM d, yyyy h:mm a')}</p>
                </div>
              </CardContent>
            </Card>
          ))}
