...
import { CalendarCheck, Check, X, Clock, CalendarOff, CheckCheck, Save, Loader2, Users, CalendarDays, GraduationCap } from 'lucide-react';
...
const STATUS_OPTIONS: Array<{
  ...
  { value: 'present', label: 'Present', icon: Check, active: 'bg-green-600 text-white border-green-600', inactive: 'text-green-700 hover:bg-green-50 border-slate-200' },
  { value: 'absent', label: 'Absent', icon: X, active: 'bg-red-600 text-white border-red-600', inactive: 'text-red-700 hover:bg-red-50 border-slate-200' },
  ...
  { value: 'leave', label: 'Leave', icon: CalendarOff, active: 'bg-blue-600 text-white border-blue-600', inactive: 'text-blue-700 hover:bg-blue-50 border-slate-200' },
];
...
export default function TeacherTakeAttendancePage() {
  ...
  // Fetch teacher's assigned batches for the dropdown
  const { data: batchesData, isLoading: batchesLoading } = useQuery<{ data: BatchOption[] }>({
    queryKey: ['teacher-batches-list-attendance'],
    queryFn: async () => {
  ...
  const {
    ...
    queryKey: ['teacher-batch-attendance', selectedBatch],
    queryFn: async () => {
  ...
  const {
    ...
    queryKey: ['teacher-attendance-existing', selectedBatch, selectedDate],
    queryFn: async () => {
  ...
  const { data: attendanceHistory } = useQuery<AttendanceHistoryRecord[], Error>({
    queryKey: ['teacher-attendance-history', selectedBatch],
    queryFn: async () => {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['teacher-attendance-existing', selectedBatch, selectedDate] });
      queryClient.invalidateQueries({ queryKey: ['teacher-attendance-history', selectedBatch] });
    },
  ...
  return (
    <DashboardLayout navSections={teacherNav} role="teacher">
      <PageHeader title="Take Attendance" description="Mark and manage student attendance for your assigned batches" />
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="py-16">
            <EmptyState
              ...
              description="Choose one of your assigned batches and a date above to load the student list and mark attendance."
            />
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="py-16">
            <EmptyState
              ...
              description="This batch has no students assigned. Students must be added to the batch before attendance can be marked."
            />
          </CardContent>
        </Card>
      )}
      ...
      {selectedBatch && !isLoadingDetail && !hasBatchError && students.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <StatCard title="Present" value={summary.present} icon={Check} color="green" />
            <StatCard title="Absent" value={summary.absent} icon={X} color="red" />
            <StatCard title="Late" value={summary.late} icon={Clock} color="amber" />
            <StatCard title="On Leave" value={summary.leave} icon={CalendarOff} color="blue" />
          </div>
